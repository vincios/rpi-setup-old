...
5. Add the client's interface IP address to the `.conf` file
    ...
    ```
    ⚠️ Don't forget to edit the `[X]` part of the address. The `Address` setting sets the peer's static IP, so make sure to increment it for each client you add to Wireguard. Make sure to give an unique IP address to any client! 

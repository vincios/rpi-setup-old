...
1. Create the file `/etc/traefik/dynamic/middlewares.yml` with the following content:
    ...
    Where the users field is an array of authorized users. Each user must be declared using the `name:hashed-password` format. See the [BasicAuth](https://doc.traefik.io/traefik/middlewares/http/basicauth/#configuration-examples) examples.
### Setup Service

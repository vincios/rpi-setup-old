...
### Add clients
Wireguard creates peer-to-peer connections (called VPN tunnels) between devices. Each peer must have a `.conf` file containing its Wireguard interface configuration (`[interface]` section) and list of `[Peer]` sections, one for each peer it will be connected to.

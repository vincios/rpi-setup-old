...
### Add clients
In a Wireguard VPN each peer must have a `.conf` file containing the peer's interface configuration. Also, each peer of the VPN tunnel must have a `[peer]` section in its `.conf` file, 
## Install Pi-hole

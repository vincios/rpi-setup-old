...
So, when your client tries to send a packet to an IP address of your LAN, that packet will be intercepted by the wireguard interface and routed through the tunnel to the server (even outside the LAN). Then, the server will forward it 
## Install Pi-hole

...
3. When the download is complete, extract the tarball, navigate to the Python source directory and run the configure script
    ...
    💡 The OPTIONAL `--enable-shared` option should be the equivalent of install the `python-dev` from apt. `python-dev` contains the header files you need to build Python extensions

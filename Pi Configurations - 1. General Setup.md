...
We will use the Wireguard `PostUp` and `PreDown` configuration settings. The `PostUp` lines will run when the WireGuard Server starts, The PreDown rules run when the WireGuard Server stops the virtual VPN tunnel

...
If the network folder is not visible, or not writeable, try this solutions:

- If windows explorer doesn't ask for password when you open the network share, open the Windows Credential Manager and, under the `Windows Credentias` tab, manually add the credentias for the network address `\\RASPBERRYPI`

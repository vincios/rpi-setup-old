...
4. Create the client's `.conf` file
   ...
   ⚠️ Make sure to give an unique IP address to any client! The `Address` setting sets the peer's static IP, so make sure to increment it for each client you add to the wiregu

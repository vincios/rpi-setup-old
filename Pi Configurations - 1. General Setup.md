...
5. Add the client's interface IP address to the `.conf` file
    ...
    The `Address` setting of the `[interface]` section sets the peer's static IP, so make sure to increment it for each client you add to Wireguard. Make sure to give an unique IP address to any client! 

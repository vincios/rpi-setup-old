...
### Add clients
In a Wireguard VPN each peer must have a `.conf` file containing the peer's interface configuration. Also, each side of the VPN tunnel must have a `[peer]` in its `.conf` file, 
## Install Pi-hole

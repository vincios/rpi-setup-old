...
2. Create the file `/etc/traefik/dynamic/dashboard.yml`, with the following content:
    ...
    >
    >> https://traefik.<DUCKDNS.DOMAIN>.duckdns.org/dashboard/
    >
...
1. Create the file `/etc/logrotate.d/traefik` (with `sudo`) and paste this content
    ...
    💡 This configuration will daily compress the logs files, keeping the last 7 days logs and deleting the oldest ones.

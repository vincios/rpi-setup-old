...
3. Clone repository
    ...
    ⚠️ It should be better to choose a branch between `bind_16.02` and `migrate-to-15.09-try2` but it should work also with `master` branchSo first try with `master` then, in case of failure, try `bind_16.02` and, as last chance, `migrate-to-15.09-try2`.

...
```bash
#! /usr/bin/env bash
# Usage: ./wg-add.sh [-h] [-f] client_id client_name server_domain
# Add new peers to the wireguard network
#
#   -h                               Show this help and exits
#   -f               OPTIONAL        Force overwrite of (eventually) existing configuration files for the same client_name
#
#   client_id        REQUIRED        Client ID. Must be an integer. Will be the client part of the client's IP
#   client_name      REQUIRED        String to identify the client
...
display_usage() {
	echo -e "Usage: $0 [-h] [-f] client_id client_name server_domain"
        echo -e "Add new peers to the wireguard network"
        echo -e "  "
        echo -e "  -h \t \t \t \t Show this help and exits"
        echo -e "  -f \t \t OPTIONAL \t Force overwrite of (eventually) existing configuration files for the same client_name. Keys will NEVER be overwritten!"
        echo -e "  "
        echo -e "  client_id \t REQUIRED \t Client ID. Must be an integer. Will be the client part of the client's IP"
        echo -e "  client_name \t REQUIRED \t String to identify the client"
...
echo "** Wireguard clients configuration script **"

client_id="$1"
ipv4="${SUBNET_IPV4}${client_id}"
ipv6="${SUBNET_IPV6}${client_id}"
...
echo -e "Generating configuration files for ${client_id}/${name} [server: ${target}]"
echo -e "  "
...
if [[ ! -f ${clientconf} && ! -f ${clientfullconf} ]]; then
    ...
    echo "AllowedIPs = ${server4}/32, ${server6}/128, ${LOCAL_LAN}" >> ${clientconf} # clients isolated from one another

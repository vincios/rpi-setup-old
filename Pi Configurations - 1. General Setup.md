...
💡 For the built from source version, the `torrc` file should be located into the `/usr/local/etc/tor/` folder, but you can check it running `tor`, one the first lines of the output should tell the exact location where the 
### obfs4proxy

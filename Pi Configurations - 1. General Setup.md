...
Now your Wireguard server is able to forward the packets incoming from the wireguard interfce `wg0` to other interfaces (like the `eth0`).
...
With this configuration, when your client tries to send a packet to an IP address into your LAN (i.e. its ), that packet will be intercepted by the wireguard interface and routed through the tunnel to the server (even outside the LAN). Then, the server will forward it to the right LAN client.

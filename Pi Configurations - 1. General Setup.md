...
7. Add the server's connection parameters as `[peer]` section of the `.conf` file
    ...
    - `PersistentKeepalive`: see the note on the [pihole](https://docs.pi-hole.net/guides/vpn/wireguard/client/#create-client-configuration) documentation. Basically, we send a keepalive packet every 25 second to prevent the (router's) NAT closes the client's NAT rule

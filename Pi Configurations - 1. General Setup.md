...
💡 For the built from source version, the `torrc` file should be located into the `/usr/local/etc/tor/` folder. You can check it running `tor`: the first few lines of the output should tell the exact location where the configuration file need to be located. If there is no `torrc` file in the 
### obfs4proxy

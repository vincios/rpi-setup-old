...
### Add clients
Wireguard creates per-to-peer each peer must have a `.conf` file containing the peer's interface configuration and list of `[peer]` sections, one for each peer il will be connected.

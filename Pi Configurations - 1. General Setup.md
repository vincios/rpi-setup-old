...
1. Create the file `/etc/traefik/dynamic/middlewares.yml` with the following content:
    ...
    Where the users field is an array of authorized users. Each user must be declared using the `name:hashed-password` format. See the [BasicAuth](https://doc.traefik.io/traefik/middlewares/http/basicauth/#configuration-examples) documentation. To generate the `name:hashed-password` string you can use any online HTPasswd Generator, like [this](https://www.web2generators.com/apache-tools/htpasswd-generator).

...
We will use the Wireguard `PostUp` and `PreDown` configuration settings. The `PostUp` lines will run when the Wireguard Server starts, the `PreDown` lines run when the Wireguard Server stops the virtual VPN tunnel

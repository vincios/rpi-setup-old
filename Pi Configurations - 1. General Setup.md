...
3. When the download is complete, extract the tarball, navigate to the Python source directory and run the configure script
    ...
    💡 The OPTIONAL `--enable-shared` option should be the equivalent of install `python-dev` from apt. `python-dev` contains the header files you need to build Python extensions
...
6. (OPTIONAL) If you have launched the command `./configure` with the `--enable-shared` option, you must add the python shared libraries to the dynamic linker
    ...
    > 💡 Use the `ldconfig` utility is equivalent to add the `/usr/local/lib` line to the `LD_LIBRARY_PATH` environment variable. But the `LD_LIBRARY_PATH` environment variable should be set on each shell login (and is not )
